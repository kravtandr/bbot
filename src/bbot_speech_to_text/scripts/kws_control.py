...
class KWSDetection(object):
	def __init__(self):
		...
		self._kws_data_pub = rospy.Publisher('/bbot/stt/kws_data', String, queue_size=10)
		self._grammar_audio_pub = rospy.Publisher('/bbot/stt/grammar_audio', AudioData, queue_size=10)
	...
	def startRecognizer(self):
		...
		rospy.Subscriber('/bbot/stt/grammar_not_found', Empty, self.grammarCheckCallback)
		rospy.spin()
