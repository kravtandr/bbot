...
class ASRControl(object):
	def __init__(self):
		...
		self._grammar_data_pub = rospy.Publisher("/bbot/stt/grammar_data", String, queue_size=10)
		self._grammar_not_found_pub = rospy.Publisher('/bbot/stt/grammar_not_found', Empty, queue_size=10)
	...
	def startRecognizer(self):
		...
		rospy.Subscriber("/bbot/stt/grammar_audio", AudioData, self.processAudio)
		rospy.spin()
